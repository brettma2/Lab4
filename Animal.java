...
public class Animal {
	 ...
	 * order of color, name, weight, and height
	 *
	 * @return The string representation of the Animal class, formatted as: "(name),
	 *         a (color)-colored animal. (weight) pounds, (height) inches.\n" Weight
	 *         and height are formatted to one decimal place.
	 */
	@Override
	...
	/**
	 * Override the equals method. Java does not inherently know how to compare two
	 * objects. Intuitively, two animals should be equal if their name, color,
	 * weight, and height are the same, but Java does not do this automatically. It
	 * must be defined. One could also define that two Animals are equal if their
	 * name and color are the same, regardless of the weight.
	 *
	 * @param other The other animal to compare to.
	 * @returns True if all fields (name, color, weight, height) are equal, false
	 *          otherwise.
	 */
	@Override
	public boolean equals(Object other) {
		...
		if ((otherAni.getColor().equalsIgnoreCase(this.getColor())
				&& (otherAni.getName().equalsIgnoreCase(this.getName())) && (otherAni.getWeight() == this.getWeight()))

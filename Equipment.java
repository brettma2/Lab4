/**
 * A class to represent items stored in Batman's utility belt. Several copies of
 * an item may appear in the Utility belt (e.g. 5 batarangs), so the Equipment
 * class keeps a count variable tracking the number of copies of the item. I.e.
 * there would be one equipment object for "batarangs", for which the count
 * variable would be equal to 5.
 *
 * @author Stephen
 * @version 2019-01-27
 */
public class Equipment {
	...
	/**
	 * Constructor for Equipment. Takes in information on the Equipment as a comma
	 * delimited string, stores info on name, count, totalWeight, totalPrice, and
	 * its description. Note that the name and count are separated by a comma, not a
	 * forward slash.
	 *
	 * @param strg Information about the Equipment in the format:
	 *             "name/count,totalWeight,totalPrice,description" The description
	 *             will not contain any commas or forward-slashes. The count is an
	 *             integer value.
	 */
	 ...
	 *
	 * @return All information about the Equipment formatted as (replacing
	 *         parentheses with member variables): "Name: (name), Number: (count),
	 *         Weight: (weight, to 2 decimal places) lbs, Price: $(price, to 2
	 *         decimal places) - (description)"
	 *
	 *         e.g. "Name: Batarang, Number: 5, Weight: 3.22 lbs, Price: $700.75 -
	 *         bat-shaped boomerangs"
	 */
	@Override
	public String toString() {
		return String.format("Name: %s, Number: %d, Weight: %.2f lbs, Price: $%.2f - %s", name, count, totalWeight,
    ...
    public boolean equals(Object other)
    {
        ...
        if ((otherEq.getName().equalsIgnoreCase(this.getName()))
				&& (otherEq.getDescription().equalsIgnoreCase(this.getDescription())) && (otherEq.getTotalWeight() == this.getTotalWeight())
				&& (otherEq.getTotalPrice() == this.getTotalPrice()) && (otherEq.getCount() == this.getCount()))
        {
			return true;
		}
